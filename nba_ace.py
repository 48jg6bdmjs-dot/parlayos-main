"""
nba_ace.py — NBA prediction engine for ParlayOS.
...
Data sources:
  - The Odds API (basketball_nba sport_key) — moneyline, spreads, totals.
...
# ── NBA team abbreviations — Odds API uses full city+name; ESPN/UI use
...
# ESPN's internal numeric team IDs — sports.core.api.espn.com/v2/sports/basketball/leagues/nba/teams
ESPN_TEAM_IDS = {
...
# League-average fallbacks (2025-26 season, updated periodically) — used
...
class NBAPredictionEngine:
    def __init__(self, api_key: str):
        ...
        print(f"NBA engine initialized with API key: {api_key[:8]}..." if api_key else
              "NBA engine initialized with NO API key — fetch_live_odds will fail")

    def fetch_live_odds(self) -> List:
        """Fetch live NBA odds from The Odds API — h2h, spreads, totals."""
    ...
    def fetch_team_advanced_stats(self, team_abbr: str) -> Dict:
        ...
        instead of raw PPG/points-allowed — the direct NBA analog of why
    ...
    def fetch_injuries(self, team_abbr: str) -> Dict:
        ...
        carry disproportionate weight vs. NFL/MLB — a single missing star
    ...
    def calculate_win_probability(self, game: Dict) -> float:
        ...
        162 — actually between MLB and NFL, but with much higher per-game
        ...
        weight — a star player missing matters (injury_edge below), but
        ...
        # ── Rating differential (pace-independent team quality) — largest
        ...
        if home_stats["rtg_has_data"] and away_stats["rtg_has_data"]:
            ...
            team_edge = (home_net_rtg - away_net_rtg) * 0.009

        # ── True shooting % — a distinct efficiency signal from net
        ...
        #    underlying "how good is this offense" fact twice — same
        ...
        if home_stats["ts_has_data"] and away_stats["ts_has_data"]:
            ts_edge = (home_stats["ts_pct"] - away_stats["ts_pct"]) * 0.15

        # ── Injury burden — weighted MORE heavily than NFL/MLB's injury
        ...
        if home_inj["has_data"] and away_inj["has_data"]:
            injury_edge = (away_inj["count"] - home_inj["count"]) * 0.012
            injury_edge = max(-0.05, min(0.05, injury_edge))

        home_court_edge = 0.02  # standard, well-established NBA home-court advantage
        # (Comparable in magnitude to MLB's 0.02; commonly cited as
        # somewhat SMALLER than NFL's in modern analytics — travel/rest
        ...
        # pass — genuinely one of the more NBA-specific, well-documented
        ...
        base_prob = (0.5 + team_edge + ts_edge + injury_edge + home_court_edge
                     + rest_edge + (game.get("market_prob", 0.5) - 0.5) * 0.15)
    ...
    def calculate_total_points(self, game: Dict, posted_total: float) -> Tuple[str, float, float]:
        ...
        no pace concept), NBA's total is dominated by PACE — two efficient
        ...
        scoring = (average of both teams' pace) × (both teams' combined
        offensive rating relative to league average) / 100 × 2, roughly
        ...
        _POINTS_SIGMA = 11.0  # NBA game-total std dev in raw points — larger
        ...
        if (home_stats["rtg_has_data"] and away_stats["rtg_has_data"]
            ...
            # per 48 min, i.e. per full game — no additional /48 scaling
            # needed since pace is already a full-game figure).
            model_total = (home_exp_per100 + away_exp_per100) * (avg_pace / 100.0)
        else:
            # Insufficient real data — fall back to the posted total as
            # the model estimate (no fabricated edge), same philosophy as
            # nfl_ace.py's calculate_total_points fallback.
...
def _picks_to_nba_games(picks: List) -> List:
    """Same field-shape contract as nfl_ace.py's _picks_to_nfl_games — see
    ...
    for idx, p in enumerate(picks):
        ...
        game = {
            ...
            'kLine': spread, 'kPick': spread_pick_str,  # SPREAD — see _picks_to_nba_games docstring
...
def export_to_html(picks: List, html_path: str) -> str:
    ...
    marker, not an NBA-specific one — see the ParlayOS.html changes in
    ...
    injection_lines = [
        "    // ── PARLAYOS NBA LIVE DATA (" + run_date + ") ──────────────────",
...
def run(html_path: str):
    """Main entry point — mirrors nfl_ace.py's run()."""
    ...
    print(f"\n✓ {len(all_games_data)} NBA games exported ({qualifying} qualify at current thresholds)")
