"""
nba_ace.py â€” NBA prediction engine for ParlayOS.
...
Data sources:
  - The Odds API (basketball_nba sport_key) â€” moneyline, spreads, totals.
...
# â”€â”€ NBA team abbreviations â€” Odds API uses full city+name; ESPN/UI use
...
# ESPN's internal numeric team IDs â€” sports.core.api.espn.com/v2/sports/basketball/leagues/nba/teams
ESPN_TEAM_IDS = {
...
# League-average fallbacks (2025-26 season, updated periodically) â€” used
...
class NBAPredictionEngine:
    def __init__(self, api_key: str):
        ...
        print(f"NBA engine initialized with API key: {api_key[:8]}..." if api_key else
              "NBA engine initialized with NO API key â€” fetch_live_odds will fail")

    def fetch_live_odds(self) -> List:
        """Fetch live NBA odds from The Odds API â€” h2h, spreads, totals."""
    ...
    def fetch_team_advanced_stats(self, team_abbr: str) -> Dict:
        ...
        instead of raw PPG/points-allowed â€” the direct NBA analog of why
    ...
    def fetch_injuries(self, team_abbr: str) -> Dict:
        ...
        carry disproportionate weight vs. NFL/MLB â€” a single missing star
    ...
    def calculate_win_probability(self, game: Dict) -> float:
        ...
        162 â€” actually between MLB and NFL, but with much higher per-game
        ...
        weight â€” a star player missing matters (injury_edge below), but
        ...
        # â”€â”€ Rating differential (pace-independent team quality) â€” largest
        ...
        # â”€â”€ True shooting % â€” a distinct efficiency signal from net
        ...
        #    underlying "how good is this offense" fact twice â€” same
        ...
        # â”€â”€ Injury burden â€” weighted MORE heavily than NFL/MLB's injury
        ...
        # somewhat SMALLER than NFL's in modern analytics â€” travel/rest
        ...
        # pass â€” genuinely one of the more NBA-specific, well-documented
    ...
    def calculate_total_points(self, game: Dict, posted_total: float) -> Tuple[str, float, float]:
        ...
        no pace concept), NBA's total is dominated by PACE â€” two efficient
        ...
        scoring = (average of both teams' pace) Ã— (both teams' combined
        offensive rating relative to league average) / 100 Ã— 2, roughly
        ...
        _POINTS_SIGMA = 11.0  # NBA game-total std dev in raw points â€” larger
        ...
        if (home_stats["rtg_has_data"] and away_stats["rtg_has_data"]
            ...
            # per 48 min, i.e. per full game â€” no additional /48 scaling
...
def _picks_to_nba_games(picks: List) -> List:
    """Same field-shape contract as nfl_ace.py's _picks_to_nfl_games â€” see
    ...
    for idx, p in enumerate(picks):
        ...
        game = {
            ...
            'kLine': spread, 'kPick': spread_pick_str,  # SPREAD â€” see _picks_to_nba_games docstring
...
def export_to_html(picks: List, html_path: str) -> str:
    ...
    marker, not an NBA-specific one â€” see the ParlayOS.html changes in
    ...
    injection_lines = [
        "    // â”€â”€ PARLAYOS NBA LIVE DATA (" + run_date + ") â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€",
        ...
        "    })();",
        "    // â”€â”€ END PARLAYOS NBA LIVE DATA â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€",
    ]
...
def run(html_path: str):
    """Main entry point â€” mirrors nfl_ace.py's run()."""
    ...
    print(f"\nâœ“ {len(all_games_data)} NBA games exported ({qualifying} qualify at current thresholds)")
